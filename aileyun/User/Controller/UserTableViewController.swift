...
//            }
            return ["身份认证", "我的消息", "意见反馈", "分享给好友", "设置"]
        }
    }
    ...
    lazy var tableView : UITableView = {
        let tv =  UITableView.init(frame: CGRect.init(x: 0, y: 0, width: SCREEN_WIDTH, height: SCREEN_HEIGHT))
    ...
    lazy var tableHeadV : UserHeadView = {
        let topMargin: CGFloat = UIDevice.current.isX ? 44 : 0
        let t = UserHeadView.init(frame: CGRect.init(x: 0, y: 0, width: SCREEN_WIDTH, height: 180 + topMargin),
                                  topMargin: topMargin)
